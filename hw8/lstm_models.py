...
class LstmBilinearAttentionTranslationModel:
    def __init__(self, name, inp_voc, out_voc,
        ...
        with tf.variable_scope(name):
            ...
            self.enc0 = tf.contrib.rnn.LSTMCell(hid_size, state_is_tuple=False)
...
class LstmRevertedBilinearAttentionTranslationModel:
    def __init__(self, name, inp_voc, out_voc,
                 emb_size, hid_size,):
...
class LstmTanhAttentionTranslationModel:
    def __init__(self, name, inp_voc, out_voc,
                 emb_size, hid_size,):
        ...
        with tf.variable_scope(name):
            ...
            self.W_omega = tf.Variable(tf.random_normal([hid_size, hid_size], stddev=0.1))
            self.b_omega = tf.Variable(tf.random_normal([hid_size], stddev=0.1))
            self.u_omega = tf.Variable(tf.random_normal([hid_size], stddev=0.1))
    ...
    def decode(self, prev_state, prev_tokens, all_h, **flags):
        ...
        all_h_w_dot = tf.reshape(tf.matmul(tf.reshape(all_h, [-1, b_t_d[2]]), self.W_omega),
                       shape=[b_t_d[0], b_t_d[1], d_a[1]])
        # For each of the timestamps its vector of size A from `v` is reduced with `u` vector
        all_h_w_dot = all_h_w_dot + self.b_omega
        all_h_w_dot = tf.tanh(all_h_w_dot)
        all_h_w_u_dot = tf.reshape(tf.matmul(tf.reshape(all_h_w_dot, [b_t_d[0] * b_t_d[1], d_a[1]]), tf.expand_dims(self.u_omega, -1)), shape=[b_t_d[0], b_t_d[1]]) # (B,T) shape
        alphas = tf.nn.softmax(all_h_w_u_dot)    # (B,T) shape also
        # Output of (Bi-)RNN is reduced with attention vector; the result has (B,D) shape
        output = tf.reduce_sum(all_h * tf.expand_dims(alphas, -1), 1)
