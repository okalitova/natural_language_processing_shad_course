...
class TranslationModel:
    ...
    def collect_statistics(self, src_tokens, trg_tokens, posterior_matrix):
        "Accumulate counts of translations from posterior_matrix[j][i][i_1] = p(a_j=i a_{j-1}=i_1|e, f)"
        ...
        for i in range(len(src_tokens)):
            for j in range(len(trg_tokens)):
                for i_1 in range(len(src_tokens)):
                    self._src_trg_counts[src_tokens[i]][trg_tokens[j]] += posterior_matrix[i][i_1]
...
class TransitionModel:
    ...
    def __init__(self, src_corpus, trg_corpus):
        ...
        self._probabilities = TransitionDict()
    ...
    def get_parameters_for_sentence_pair(self, src_length):
        ...
        return np.array([[self._probabilities[i][(i_1, src_length)]
                          for i in range(src_length)] for i_1 in range(src_length)])
    ...
    def collect_statistics(self, src_length, transition_posteriors):
        ...
        for i in range(src_length):
            for i_1 in range(src_length):
                self._counts[(i_1, src_length)][i] += transition_posteriors[i_1][i]

    def recompute_parameters(self):
        "Reestimate the parameters and reset counters."
        self._probabilities = TransitionDict()
        for i_1_I, i_1_I_counts in self._counts.items():
            i_1, I = i_1_I
            ssum = sum([count for count in i_1_I_counts.values()])
            for i, i_counts in i_1_I_counts.items():
                self._probabilities[i][(i_1, I)] = i_counts / ssum
